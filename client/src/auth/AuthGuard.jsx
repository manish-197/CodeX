  ...
  if (isLoading) {
    return (
      <div className="py-20 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-semibold text-deep-teal dark:text-sky-mist">
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-xl mx-auto py-12 px-4 animate-fadeIn">
        <div className="glass-card p-8 sm:p-10 text-center space-y-6 border-2 border-terracotta/40 shadow-2xl">
          
          <div className="w-20 h-20 rounded-3xl bg-gradient-to-tr from-terracotta to-sun-gold text-white flex items-center justify-center mx-auto shadow-xl">
            <Lock className="w-10 h-10" />
          </div>

          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-terracotta/15 text-terracotta text-xs font-bold uppercase tracking-wider">
              <ShieldAlert className="w-4 h-4" />
              <span>Authentication Required • लॉगिन आवश्यक आहे</span>
            </div>
            <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-deep-teal dark:text-sky-mist">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <button
              ...
              className="w-full sm:w-auto btn-terracotta py-3.5 px-8 text-sm flex items-center justify-center gap-2 shadow-lg"
            >
  ...
  if (requiredRole && currentUser?.role !== requiredRole) {
    return (
      <div className="max-w-lg mx-auto py-12 px-4 animate-fadeIn">
        <div className="glass-card p-8 text-center space-y-4 border border-alert-crimson/30">
          <div className="w-14 h-14 rounded-2xl bg-alert-crimson/15 text-alert-crimson flex items-center justify-center mx-auto">
          ...
          {onNavigateHome && (
            <button onClick={onNavigateHome} className="btn-teal text-xs py-2 px-6 mt-2">
              Go to Home Screen
            </button>
          )}
        </div>
